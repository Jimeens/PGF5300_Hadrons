Considerando o espalhamento elástico elétron-múon, os tensores leptônico e hadrônico são dados por
    \begin{equation*}
        L^{\alpha\beta} = \dfrac{1}{2}\sum_{s,s'}\qty[\bar{u}_{e}(k,s)\gamma^{\alpha}u_{e}(k',s')]^{\ast}\bar{u}_{e}(k,s)\gamma^{\beta}u_{e}(k',s')
    \end{equation*}
e
    \begin{equation*}
        W_{\alpha\beta} = \dfrac{1}{2}\qty{\sum_{S,S'}\qty[\bar{u}_{\mu}(k',S')\gamma_{\alpha}u_{\mu}(k,S)]^{\ast}\bar{u}_{\mu}(k,S)\gamma_{\beta}u_{\mu}(k',S')}\delta^{4}(2p\cdot q + q^{2})
    \end{equation*}
...
Fica claro então, por conservação de momento, que $q = k - k' = p' - p$. Considerando a estrutura principal que ocorre nos dois tensores utilizando espinores arbitrários $u$ e $\bar{u}$, podemos notar que termos do tipo $\bar{u}\gamma^{\mu}u$ são números complexos, o que faz com que 
    \begin{align*}
        [\bar{u}(k,s)\gamma^{\mu}u(k',s')]^{\ast} &\eq [\bar{u}(k,s)\gamma^{\mu}u(k',s')]^{\dagger} \\
        &\eq [u^{\dagger}(k,s) \gamma^{0} \gamma^{\mu} u(k',s')]^{\dagger} \\
        &\eq u^{\dagger}(k',s') (\gamma^{\mu})^{\dagger} \gamma^{0} u(k,s)
    \end{align*}

Usando então que $(\gamma^{\mu})^{\dagger} = \gamma^{0}\gamma^{\mu}\gamma^{0}$ e $(\gamma^{0})^{2} = \mathds{1}$, obtemos
    \begin{align*}
        [\bar{u}(k,s)\gamma^{\mu}u(k',s')]^{\ast} &\eq u^{\dagger}(k',s')\gamma^{0} \gamma^{\mu} \gamma^{0} \gamma^{0} u(k,s) \\
        &\eq \bar{u}(k',s') \gamma^{\mu} u(k,s)
    \end{align*}

Portanto os tensores podem ser escritos sob a forma
    \begin{equation*}
...
Os tensores ainda possuem uma estrutura semelhante, então tratar somente uma delas é o suficiente para o cálculo da contração $L^{\alpha\beta}W_{\alpha\beta}$. A estrutura em questão é dada por
    \begin{equation*}
        \dfrac{1}{2} \sum_{s,s'} 
        \bar{u}(k',s') \gamma^{\alpha} u(k,s)
        \bar{u}(k,s) \gamma^{\beta} u(k',s')
    \end{equation*}
onde, para simplificar novamente a notação, os índices $e$ e $\mu$ foram removidos. A expressão acima pode ser reescrita em componentes, isto é
    \begin{equation*}
...
Note que $q^{2}$ pode ser reescrito em termos de um ângulo $\theta$ formado pelos vetores $\vb{k}$ e $\vb{k}'$, isto é
    \begin{align*}
        q^{2} = -2(k'\cdot k) &\eq -2(E'E - \vb{k}'\cdot\vb{k}) \\
