...
Podemos escrever
    \begin{align*}
        E - E' = \dfrac{1}{M}(ME - ME') = \dfrac{1}{M}(p\cdot k - p\cdot k') = \dfrac{1}{M}p\cdot(k - k') = \dfrac{p\cdot q}{M}
    \end{align*}

Então definindo $\nu \coloneqq \dfrac{p\cdot q}{M} = E - E'$ e $q^{2} \equiv -Q^{2}$, concluímos que a contração entre os tensores, no referencial do laboratório, assume a forma
