\textbf{\textsc{Nota:}} as setas de momento do enunciado original não condiziam com um diagrama $t$, que era a proposta do exercício (mostrar \textit{crossing simmetry} do diagrama $t$, conforme conversado com o professor), portanto alterei as setas de momento para condizer com um diagrama $t$.
...
Então o diagrama que estamos buscando é o terceiro da lista acima, que representa a contribuição
    \begin{center}
        \begin{tikzpicture}
            \node at (-0.9,-2.2) {$\mathbb{J}_{2} \coloneqq \dfrac{(-i\lambda)^{2}}{2}J_{2} =$};
            \hspace{1.5cm}
            \begin{feynman}
                \vertex (i1);
...
Conforme mostrado na Aula 11, vale a igualdade
    \begin{equation}\label{eq: integral dimreg}
