...
\begin{tikzpicture}
    ...
    \begin{feynman}
        \vertex (i1);
        \vertex [below right=0.5*3cm and 0.5*2cm of i1, dot] (i2) {};
        \vertex [below left=0.5*1.5cm and 0.5*2cm of i2] (i3);
        ...
        \diagram* {
            (i1) -- [plain, half left, looseness=0.425, rmomentum'={[arrow shorten=0.45]$p_{3}$}] (i5) -- [plain, rmomentum={[arrow shorten=0.7]$p_{4}$}] (i6),
            (i2) -- [plain, half left, looseness=1.25] (i5),
            (i5) -- [plain, half left, looseness=1.25] (i2),
            (i4) -- [plain, half right, looseness=0.425, momentum={[arrow shorten=0.45]$p_{1}$}] (i2) -- [plain, momentum'={[arrow shorten=0.7]$p_{2}$}] (i3),
        };
    \end{feynman}
\end{tikzpicture}
...
Então o diagrama que estamos buscando é o terceiro da lista acima, que representa a contribuição
    \begin{equation*}
        \mathbb{J}_{2} \coloneqq \dfrac{(-i\lambda)^{2}}{2}J_{2}
    \end{equation*}

Utilizando as regras de Feynman para o modelo $\lambda\phi^{4}$, o diagrama representado por $\mathbb{J}_{2}$ em regularização dimensional é dado por
    \begin{equation*}
        \mathbb{J}_{2} = -\dfrac{\lambda^{2}}{2} \mu^{4-\mathsf{D}} \int 
        \dfrac{1}{(2\pi)^{\mathsf{D}}}
        \dfrac{i}{(-p_{3}+k)^{2} - m^{2} + i\varepsilon}
        \dfrac{i}{(p_{1}-k)^{2} - m^{2} + i\varepsilon}
        \dd[\mathsf{D}]{k}
    \end{equation*}

Fazendo uma mudança de variável $k \mapsto k + p_{3}$, que mantém a integral invariante, temos
    \begin{equation*}
        \mathbb{J}_{2} = \dfrac{\lambda^{2}}{2} \mu^{4-\mathsf{D}} \int 
        \dfrac{1}{(2\pi)^{\mathsf{D}}}
        \dfrac{1}{k^{2} - m^{2} + i\varepsilon}
        \dfrac{1}{(p_{1} - k - p_{3})^{2} - m^{2} + i\varepsilon}
        \dd[\mathsf{D}]{k}
    \end{equation*}

Sendo então $D_{1} \coloneqq k^{2} - m^{2} + i\varepsilon$ e $D_{2} \coloneqq (p_{1} - k - p_{3})^{2} - m^{2} + i\varepsilon$, obtemos
    \begin{equation*}
        \mathbb{J}_{2} = \dfrac{\lambda^{2}}{2} \mu^{4-\mathsf{D}} \int 
        \dfrac{1}{(2\pi)^{\mathsf{D}}}
        \dfrac{1}{D_{1}D_{2}}
...
onde definimos $D_{T} \coloneqq (1-a)D_{1} + aD_{2}$. Desenvolvendo $D_{T}$, obtemos
    \begin{align*}
        D_{T} &\eq (1-a)(k^{2} - m^{2} + i\varepsilon) + a\qty[(p_{1} - k - p_{3})^{2} - m^{2} + i\varepsilon] \\
        &\eq k^{2} - m^{2} + i \varepsilon - ak^{2} + {\color{orange}am^{2}} - {\color{red!70!black}ia\varepsilon} + a(p_{1} - k - p_{3})^{2} - {\color{orange}am^{2}} + {\color{red!70!black}ia\varepsilon} \\
