...
A integral restante nesta expressão não converge no sentido usual, mas sim no sentido de distribuições, de modo que seu resultado é dado por
    \begin{align*}
        \int_{0}^{\infty}e^{i(q_{0}+p_{0}-p_{X0})z_{0}}\dd{z_{0}} &\eq \pi\delta(q_{0} + p_{0} - p_{X0}) + i\mathsf{VP}\qty(\dfrac{1}{q_{0} + p_{0} - p_{X0}})
    \end{align*}

Onde $\mathsf{VP}(\cdots)$ é o valor principal de Cauchy\footnote{O valor principal de Cauchy pode não ser muito conhecido, porém na resolução deste exercício ele não vai precisar ser levado em conta, dado que eles vão ser relevantes apanas para contribuição da parte real da amplitude.}. Logo
    \begin{equation*}
...
No caso da segunda integral, $I_{2}$, (com a Heaviside em $-z_{0}$), teremos
    \begin{align*}
        ...
        &\eq \int_{0}^{\infty}e^{i(-q_{0}-p_{X0}+p_{0})z_{0}}\dd{z_{0}}\int_{-\infty}^{\infty}e^{i(-\vb{q}-\vb{p}_{X}+\vb{p})\cdot\vb{z}}\dd[3]{z} \\
...
A delta segunda de Dirac dessa expressão impõe que a conservação do 4-momento é $p_{X} = p - q$. Sendo então $\nu = \dfrac{p\cdot q}{m}$ e $q^{2} = -Q^{2}$, com $Q>0$ sendo o momento transferido, temos
    \begin{align*}
        p_{X}^{2} = (p - q)^{2}
        = p^{2} - 2p\cdot q + q^{2}
        = m^{2} - 2m\nu - Q^{2}
...
No regime do DIS, o momento transferido $Q^{2} \to \infty$, o que faria com que $p_{X}^{2} < 0$, portanto não devem existir estados intermediários na camada de massa, implicando em $\delta^{4}(-q - p_{X} + p) = 0$. Portanto
    \begin{equation*}
        \text{Im}[T_{\mu\nu}] = \pi(2\pi)^{3}\sum_{X}\bra{p}J_{\mu}(0)\ket{X}\bra{X}J_{\nu}(0)\ket{p}\delta^{4}(q + p - p_{X})
    \end{equation*}

O termo da direita pode ser identificado como o tensor hadrônico $W_{\mu\nu}$ apresentado na Aula 17 (a menos de um fator $2\pi$), concluindo que
...
\noindent \textbf{(b)} Ao interirmos um conjunto completo de estados intermediários $\ket{X}$ entre as correntes eletromagnéticas, temos
    ...
    \begin{align*}
        ...
        &\eq \sum_{X}\bra{p}J_{\mu}(0)\ket{X}\bra{X}J_{\nu}(0)\ket{p} \int e^{i(q + p_{X} - p)z} \dd[4]{z} \\
        &\eq (2\pi)^{3}\sum_{X} \bra{p} J_{\mu}(0)\ket{X}\bra{X} J_{\nu} \ket{p} \delta^{4}(q + p_{X} - p)
    \end{align*}

Como a delta de Dirac é par, $\delta^{4}(q + p_{X} - p) = \delta^{4}(-q - p_{X} + p)$, então pelo mesmo argumento do item (a), essa quantidade é nula, concluindo que
